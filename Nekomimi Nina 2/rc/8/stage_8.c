...
void
n_nn2_stage_8_init( n_nn2 *p )
{

	n_nn2_stage *s = &n_nn2_stage_8;
	...
	n_nn2_rc_load_map( @"rc/8/map/0" , &s->map );
	n_nn2_rc_load_map( @"rc/8/map/1" , &s->map_queue );
	n_nn2_rc_load_map( @"rc/8/map/2" , &s->map_dokan );
	//n_nn2_rc_load_map( @"rc/1/map/15", &s->map_bell  );
	...
	n_nn2_stage_mountain_set( p, s, s->map_sx / 8 );
